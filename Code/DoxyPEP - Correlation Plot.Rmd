...
metadata<-read.csv("../Inputs/final_samples.csv", header=TRUE) %>% filter(paired=="Y")
...
#### Gather AMR data ####
amr_data<-read.csv("../Inputs/Resistome/final_AMR.csv") %>%
  filter(sample_name %in% metadata$sample_name)
...
amr_tet <- amr_noneg %>% filter(abx_class=="tetracycline") %>% mutate(sample_name=toupper(sample_name))
amr_wide<-amr_tet %>% pivot_wider(names_from = gene_name, values_from = dpm, id_cols = sample_name, values_fill = 0)

### subset amr doxy ###
amr_doxy<-amr_wide %>% filter(sample_name %in% doxy0$sample_name| sample_name %in% doxy6$sample_name)
genes <-amr_doxy[c(2, 3:length(amr_doxy))]

### subset amr soc ###
amr_soc<-amr_wide %>% filter(sample_name %in% soc0$sample_name| sample_name %in% soc6$sample_name)
genes_soc <-amr_soc[c(2, 3:length(amr_soc))]

# format gene names #
formatted_genes<-genes %>% mutate(EmrK=`EMRK`, EmrY=`EMRY`, MdfA = `ESCHERICHIA COLI MDFA`, MepA=`MEPA`, MexK=`MEXK`, MexL=`MEXL`,`Tet(32)`=`TET(32)`, `Tet(33)`=`TET(33)`, `Tet(37)`=`TET(37)`, `Tet(38)`=`TET(38)`, `Tet(40)`=`TET(40)`, `Tet(44)`=`TET(44)`, `Tet(A)`=`TET(A)`, `TetA(46)`= `TETA(46)`, `TetA(60)`=`TETA(60)`, `TetA(P)` = `TETA(P)`, `Tet(B)`=`TET(B)`, `TetB(46)`= `TETB(46)`, `TetB(P)`= `TETB(P)`, `Tet(D)`=`TET(D)`, `Tet(K)`=`TET(K)`, `Tet(L)`=`TET(L)`,`Tet(M)`=`TET(M)`,`Tet(O)`=`TET(O)`, `Tet(O/32/O)`=`TET(O/32/O)`, `Tet(O/M/O)`=`TET(O/M/O)`,`Tet(O/W)`=`TET(O/W)`, `Tet(O/W/32/O)`=`TET(O/W/32/O)`, `Tet(O/W/O)`=`TET(O/W/O)`, `Tet(Q)`=`TET(Q)`, `Tet(S)`=`TET(S)`, `Tet(T)`=`TET(T)`, `Tet(W)`=`TET(W)`, `Tet(W/32/O)`=`TET(W/32/O)`,`Tet(W/N/W)`=`TET(W/N/W)`, `Tet(X)`=`TET(X)`+ `TET(X1)` + `TET(X3)` + `TET(X4)`+ `TET(X5)`, `Tet(Z)`=`TET(Z)`)

# collapse genes doxy- revised after seeing the results of the plot # 
collapsed_genes<-formatted_genes %>% select('EmrK', 'EmrY', 'MdfA', 'MepA', 'MexK', 'MexL', 'Tet(32)', 'Tet(33)', 'Tet(37)', 'Tet(38)', 'Tet(40)','Tet(44)','Tet(A)', 'TetA(46)', 'TetA(60)', 'TetA(P)', 'Tet(B)', 'TetB(46)', 'TetB(P)', 'Tet(D)', 'Tet(K)', 'Tet(L)', 'Tet(M)', 'Tet(O)', 'Tet(O/32/O)', 'Tet(O/M/O)', 'Tet(O/W)', 'Tet(O/W/32/O)', 'Tet(O/W/O)', 'Tet(Q)', 'Tet(S)', 'Tet(T)', 'Tet(W)', 'Tet(W/32/O)', 'Tet(W/N/W)', 'Tet(X)', 'Tet(Z)')
...
# extract tet amr genes #
amr_genes_doxy<-formatted_amr_doxy %>% mutate(sample_name = sample_name) %>%
  select(sample_name, colnames(collapsed_genes))
amr_genes_soc<-formatted_amr_soc %>% mutate(sample_name = sample_name) %>%
  select(sample_name, colnames(collapsed_genes))
...
cor_matrix0 <- cor(cd0[-c(0:1)], method = "spearman")[1:37, 38:87]
cor_matrix6 <- cor(cd6[-c(0:1)], method = "spearman")[1:37, 38:87]

# Create a data frame for p-values
p_values0 <- cor_pmat(cd0[-c(0:1)])[1:37, 38:87]
p_values6 <- cor_pmat(cd6[-c(0:1)])[1:37, 38:87]
...
row.order <- c("Tet(40)","Tet(L)","EmrY","EmrK","Tet(B)","MdfA","Tet(K)","Tet(A)","TetA(46)","TetB(46)","TetA(P)","MexK","Tet(33)","MexL","Tet(Z)","TetA(60)","MepA","Tet(38)","Tet(D)","Tet(X)","Tet(37)","Tet(M)","Tet(Q)","Tet(W)","Tet(O/M/O)","Tet(O)","Tet(W/N/W)","Tet(O/W/32/O)","Tet(32)", "Tet(O/32/O)","Tet(W/32/O)","Tet(O/W/O)","Tet(O/W)","Tet(44)","TetB(P)","Tet(T)","Tet(S)")
...
## Supplemental Figure 7A and B: Correlation plot at D0 and M6

```{r correlation with AMR at D0 and M6 - DNA seq}

##### PUB FIGURES DOXY 0/6 tet v genus #### 
day0corrplot <- corrplot(cor_matrix0_fin, method="circle", type='full', tl.col='black',  p.mat=p_values0_fin, sig.level = c(0.01, 0.05), pch.cex = 0.6, insig = 'label_sig', pch.col = 'grey20', tl.cex = 0.6, col=colorRampPalette(c("#3B9AB2","#EEEEEE","#F21A00"))(200), na.label=" ")

month6corrplot <- corrplot(cor_matrix6_fin, method="circle", type='full', tl.col='black', p.mat=p_values6_fin, sig.level = c(0.01, 0.05), pch.cex = 0.6, insig = 'label_sig', pch.col = 'grey20', tl.cex = 0.6, col=colorRampPalette(c("#3B9AB2","#EEEEEE","#F21A00"))(200), na.label=" ")

```

## Supplemental Figure 7C: Change in correlation strength between at D0 and M6 in the DP arm
...
delta_matrix_pos <- (cor_matrix6_fin-cor_matrix0_fin)*p_values6_sig

diffcorrplot <- corrplot(delta_matrix_pos, method="square", is.corr = FALSE, type='full', tl.col='black', pch.cex = 0.6, insig = 'label_sig', pch.col = 'grey20', tl.cex = 0.6, col=colorRampPalette(c("#3B9AB2","#EEEEEE","#F21A00"))(200), na.label=" ")
