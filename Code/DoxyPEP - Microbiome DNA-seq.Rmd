...
reports_path = "../Inputs/Microbiome/DNA/sample_taxon_reports_combined"
...
ptid<-read.csv("../Inputs/Microbiome/DNA/ptid_061423.csv", header=TRUE)
...
``` {r build reports}
# Make Genus & Species Reports #
read_reports(reports_path, tax_level=2) %>%
  dplyr::filter(sample_name %in% sample_overviews$sample_name) -> genus_reports

read_reports(reports_path, tax_level=1) %>%
  dplyr::filter(sample_name %in% sample_overviews$sample_name) -> species_reports

read_reports(reports_path) %>%
  dplyr::filter(sample_name %in% sample_overviews$sample_name) %>%
  dplyr::mutate(genus_tax_id=dplyr::if_else(genus_tax_id < 0, tax_id, genus_tax_id)) %>%
  dplyr::left_join(genus_reports %>%
  dplyr::select(sample_name, tax_id, nt_count) %>%
  dplyr::rename(genus_tax_id=tax_id, genus_nt_count=nt_count)) %>%
  dplyr::mutate(genus_nt_count=dplyr::if_else(is.na(genus_nt_count),nt_count, genus_nt_count)) -> reports
...
``` {r data cleaning}
# extract sample_ID from full sample name #
sample_overviews_batch <-sample_overviews %>%
  mutate(sample_ID = sapply(strsplit(sample_name,"_"),"[[",2))
```
...
reports_filterbg <- filter_background(reports, controls,batches=batches,normalization=ercc_norm)

reports_filterbg_species <- filter_background(species_reports, controls,batches=batches,normalization=ercc_norm)

### Adjust p values ###
reports_filterbg_padj<-reports_filterbg %>%
  dplyr::group_by(sample_name) %>%
  dplyr::mutate(p_adj=p.adjust(p_val, method="fdr")) %>%
  dplyr::ungroup() 

reports_filterbg_species_padj<-reports_filterbg_species %>%
  dplyr::group_by(sample_name) %>%
  dplyr::mutate(p_adj=p.adjust(p_val, method="fdr")) %>%
  dplyr::ungroup() 
...
``` {r ercc}
### ERCC calculations ###
sample_overviews_ercc<-
  for(i in 1:nrow(sample_overviews)){
    ercc_counts<-sample_overviews[i,9]
    total_counts<-sample_overviews[i,6]
    ercc_input<-0.000000000025
    sample_overviews[i,36]<-(ercc_input/(ercc_counts/total_counts))-ercc_input
    sample_overviews[i,37]<-sample_overviews[i,36]*1000000000000
  }
```
...
p<-ggplot(reports_6, aes(x=as.character(visitcode), y=norm_bacterial_mass2, color=arm)) +
  ...
  geom_boxplot(alpha=0.5, outlier.shape=NA, fill=NA, width=0.6) + # scale_y_log10() +
...
stat.test_onlydoxy_ng <-trimmed_3 %>% 
  ...
  add_significance("p.adj") %>% 
  mutate(y.position = 3.5, p_char = case_when(p<0.1 ~ paste("p.adj =",format(p.adj, scientific = TRUE, digits = 2)), TRUE ~ paste("p =", as.character(format(round(p.adj, digits=2), nsmall =2)))))
...
stat.test_onlydoxy_ct <-trimmed_3 %>% 
  ...
  add_significance("p.adj") %>% 
  mutate(y.position = 1, p_char = case_when(p<0.1 ~ paste("p.adj =",format(p.adj, scientific = TRUE, digits = 2)), TRUE ~ paste("p =", as.character(format(round(p.adj, digits=2), nsmall =2)))))  
...
stat.test_onlydoxy_mg <-trimmed_3 %>% 
  ...
  adjust_pvalue(method = "bonferroni") %>%
  add_significance("p.adj") %>% 
  mutate(y.position = 1.75, p_char = case_when(p<0.1 ~ paste("p.adj =",format(p.adj, scientific = TRUE, digits = 2)), TRUE ~ paste("p =", as.character(format(round(p.adj, digits=2), nsmall =2)))))  
...
stat.test_onlydoxy_cd <-trimmed_3 %>% 
  ...
  adjust_pvalue(method = "bonferroni") %>%
  add_significance("p.adj") %>% 
  mutate(y.position = 4, p_char = case_when(p<0.1 ~ paste("p.adj =",format(p.adj, scientific = TRUE, digits = 2)), TRUE ~ paste("p =", as.character(format(round(p.adj, digits=2), nsmall =2)))))  
